...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public class Movement : MovementBehaviour
    {
        ...
        [Header("Jumping")]  // NOUVEAU

        [Tooltip("Force applied when jumping."), SerializeField]
        private float jumpForce = 5.0f;  // NOUVEAU

        [Tooltip("Gravity multiplier when falling."), SerializeField]
        private float gravityMultiplier = 2.5f;  // NOUVEAU
        ...
        private void MoveCharacter()
        {
            ...
            #region Jump Logic  // NOUVEAU

            //Check if jump input is pressed and player is grounded
            if (playerCharacter.GetInputJump() && grounded)
            {
                //Apply jump force using impulse mode for instant velocity change
                rigidBody.AddForce(Vector3.up * jumpForce, ForceMode.Impulse);
            }

            //Apply extra gravity when falling for better jump feel
            if (rigidBody.linearVelocity.y < 0 && !grounded)
            {
                rigidBody.AddForce(Vector3.down * gravityMultiplier, ForceMode.Acceleration);
            }

            #endregion

            //Update Velocity - CONSERVE LA COMPOSANTE Y!
